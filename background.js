...
function getGroupTitle(tab) {
  ...
  return domain ? getSecDomain(tab.url) : tab.title
}

async function mergeSameTabs() {
  const tabs = await chrome.tabs.query({ windowId: chrome.windows.WINDOW_ID_CURRENT })
  let tabGroups = {};
  tabs.forEach((tab) => {
    // 在线tab
    if (tab.url) {
      if (!tabGroups[tab.url]) {
        tabGroups[tab.url] = tab;
      } else {
        chrome.tabs.remove(tab.id);
      }
    } else {
      // 离线tab
      if (!tabGroups[tab.title]) {
        tabGroups[tab.title] = tab;
      } else {
        chrome.tabs.remove(tab.id);
      }
    }
...
async function querySameTabs(url) {
  const domain = getSecDomain(url)
  let tabs
...
async function groupAllTabs() {
  ...
  // 调用 chrome API 进行 tabs 分组，展开当前tab组
  let [curTab] = await chrome.tabs.query({ active: true, lastFocusedWindow: true })
  for (const groupTitle in tabGroups) {
    const tabIds = tabGroups[groupTitle].map((tab) => tab.id)
    chrome.tabs.group({ tabIds }).then((groupId) => {
      const collapsed = !tabIds.includes(curTab.id)
      chrome.tabGroups.update(groupId, { title: groupTitle, collapsed })
    })
  }
}

async function groupTab(tab, windowId) {
  const groupTitle = getGroupTitle(tab)
  ...
  if (tabGroups && tabGroups.length > 0) {
    ...
    const tabIds = [...tabs.map(tab => tab.id), tab.id]
...
async function activeGroup(tabId, windowId) {
  const tabGroups = await chrome.tabGroups.query({ windowId: windowId || chrome.windows.WINDOW_ID_CURRENT })
  for (const group of tabGroups) {
    const tabs = await chrome.tabs.query({ groupId: group.id })
    const collapsed = !tabs.find(tab => tab.id === tabId)
